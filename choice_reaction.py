...
class ChoiceReactionTest(QObject):
    ...
    def paint(self, painter):
        ...
        if self.state == 'waiting_start':
            painter.drawText(QRectF(0, 0, w, 40), Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignHCenter,
                             "Нажмите и удерживайте «Старт». Затем выберите целевой стимул.")
                ...
                if self.has_left_button_area and self.release_time:
                    ...
                    if clicked_stimulus is not None:
                        ...
                        if clicked_stimulus['is_target']:
                            ...
                            print(f"DEBUG успешная попытка (Реакция выбора):")
    ...
    def show_stimuli(self):
        try:
            ...
            w, h = self.parent.width(), self.parent.height()
            positions = []
            for _ in range(2):  # 2 стимула: 1 целевой + 1 дистрактор
                for _ in range(100):
                    ...
                    x = w / 2 + (80 if len(positions) == 0 else -80)
                    y = h / 2 - 50
            ...
            # Дистрактор: отличается по форме ИЛИ цвету
            if random.choice([True, False]):
                # Меняем только форму
                shape = random.choice([s for s in shapes if s != target_shape])
                color = target_color
            else:
                # Меняем только цвет
                shape = target_shape
    ...
    def _color_to_str(self, color):
        ...
        if color == QColor(255, 179, 186):  # red
            return 'красный'
        elif color == QColor(181, 234, 215):  # green
            return 'зеленый'
        elif color == QColor(255, 224, 179):  # yellow
            return 'желтый'
        elif color == QColor(199, 206, 234):  # blue
            return 'синий'
        return 'неизвестный'
    ...
    def _emit_result(self, latency, motor_time, total_rt, correct, anticipation, delay, 
        ...
        print(f"DEBUG результат попытки (Реакция выбора):")
