...
class GorbovShulteTest(QObject):
    ...
    def paint(self, painter):
        ...
        if self.state == 'waiting_start':
            ...
            lines = [
                ...
                "Подсказки: подсвечивается следующее число"
            ]
            ...
            # 2. ПРАВАЯ ПАНЕЛЬ - инструкции
            right_panel_x = w - panel_width - 20
            right_panel_y = start_y
            right_panel_height = panel_height
            
            # Рисуем стилизованную панель
            content_rect = self.draw_panel(painter, right_panel_x, right_panel_y,
                                         panel_width, right_panel_height,
                                         "ИНСТРУКЦИЯ", QColor(80, 100, 180))
            ...
            # Основная инструкция
            instruction_text = "Найдите числа в последовательности:"
            ...
            play_rect = QRectF(content_rect.x(), content_rect.y() + 120,
                             content_rect.width(), 40)
            play_text = "Кликните на подсвеченное\nчисло в таблице"
            painter.drawText(play_rect, Qt.AlignmentFlag.AlignCenter, play_text)
            
            # 3. ТАБЛИЦА ПО ЦЕНТРУ (между панелями)
            # Проверяем, не перекрывают ли панели таблицу
            if left_panel_x + panel_width > start_x:
                # Если перекрывают, смещаем таблицу правее
                overlap = (left_panel_x + panel_width) - start_x
                start_x = left_panel_x + panel_width + 20
            
            if right_panel_x < start_x + table_width:
                # Если перекрывают, смещаем таблицу левее
                overlap = start_x + table_width - right_panel_x
                start_x = right_panel_x - table_width - 20
            
            self.draw_table(painter, start_x, start_y)

    def draw_table(self, painter, start_x, start_y):
        """Отрисовка таблицы с подсветкой текущей цели"""
            ...
            for col in range(self.grid_size):
                ...
                if idx < len(self.numbers):
                    ...
                    if color_name == 'red':
                        ...
                        normal_bg = QColor(255, 240, 240)
                        highlight_bg = QColor(255, 200, 200, 200)
                    else:
                        text_color = QColor(60, 60, 60)
                        normal_bg = QColor(245, 245, 245)
                        highlight_bg = QColor(220, 220, 220, 200)
                    
                    # Подсветка текущей цели
                    is_target = (number == target_number and color_name == target_color)
                    
                    # Фон ячейки
                    if is_target:
                        painter.setBrush(QBrush(highlight_bg))
                    else:
                        painter.setBrush(QBrush(normal_bg))
    ...
    def mousePressEvent(self, event):
        if self.state == 'waiting_start':
            ...
            # Вычисляем start_x заново, как в paint()
            panel_width = 250
            left_panel_x = 20
            right_panel_x = w - panel_width - 20
            start_x = w/2 - table_width/2
    ...
    def finish_test(self):
        ...
        # Разделяем время на красные и черные
        ...
        result = {
            'test_name': 'Таблицы Горбова-Шульте',
            'total_rt': total_time,
